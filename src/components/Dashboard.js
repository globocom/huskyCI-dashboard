		...
		fetch(huskyRoute)
			.then(response => {
				...
				if (huskyRoute === huskyCIAuthorRoute) {
					response.then(authorResultJSON => {
				...
				if (huskyRoute === huskyCIAnalysisRoute) {
					response.then(analysisResultJSON => {
						let [
							numFailedResult,
							numWarningResult,
							numPassedResult,
				...
				if (huskyRoute === huskyCILanguageRoute) {
					...
					response.then(languageResultJSON => {
				...
				if (huskyRoute === huskyCIContainerRoute) {
					...
					response.then(containerResultJSON => {
				...
				if (huskyRoute === huskyCIRepositoryRoute) {
					...
					response.then(repositoryResultJSON => {
...
const Row = ({ children, type }) => (
	<div
		className={`dashboard__row ${
			type === 'charts' ? 'dashboard__chart-container' : null
		} ${type === 'cards' ? 'dashboard__card-container' : null}`}
	>
		{children}
	</div>
);
